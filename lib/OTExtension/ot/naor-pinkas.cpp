...
namespace semihonestot {
	...
	BOOL NaorPinkas::ReceiverECC(int nSndVals, int nOTs, CBitVector& choices,
		...
		ebrick bg, bc;
#else
		std::vector<EC2> PK_sigma(nOTs);
		std::vector<EC2> pDec(nOTs);
		std::vector<EC2> pC(nSndVals);
		EC2 PK0, ecctmp, invtmp, g;
		irand((unsigned int)time(NULL) - 100);
		ebrick2 bg, bc;
#endif
		vector<Big> pK(nOTs);
		Big bigtmp, x, y, xtmp, ytmp;
		...
		for (int k = 0; k < nOTs; k++) {
			...
			Miracl_InitPoint(&PK_sigma[0] + k, xtmp, ytmp);//PK_sigma[k] = EC2(xtmp, ytmp);
		...
		for (int u = 0; u < nSndVals; u++) {
			ByteArrayToPoint(&pC[0] + u, coordSize, pBufIdx);//mpz_import(pC[u], state.field_size, 1, sizeof(pBuf[0]), 0, 0, pBufIdx);
			pBufIdx += (coordSize + 1);
			//epoint2_norm(pC[u].get_point());
		}
		//cout << "pC[0] = " << pC[0] << endl;
		Miracl_InitBrick(&bc, &pC[0]);
		...
		for (int k = 0; k < nOTs; k++) {
			...
			Miracl_InitPoint(&pDec[0] + k, xtmp, ytmp);//pDec[k] = EC2(xtmp, ytmp);
...
#else
		//EC2 * pCr = (EC2 *)alloca(sizeof(EC2) * nSndVals);
		//EC2 * pC = (EC2 *)alloca(sizeof(EC2) * nSndVals);
		EC2 ecctmp, PK0r, invtmp, g;
		vector<EC2> pCr(nSndVals);
